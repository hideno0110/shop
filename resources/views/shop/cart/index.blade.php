...
<div class="carts-wrapper">
  <table>
    ...
    @foreach($carts as $cart)
      <tr>
        <td class="cart_item_image"><img src="/images/{{ $cart->item->img }}"></td>
        <td class="cart_item_name">{{ $cart->item->name }}</td>
        <td class="cart_item_delete">
          <form method="post" action="{{ action('CartController@delete',$cart->id) }}">
            {{ csrf_field() }}
            {{ method_field('delete') }}
            <input type="submit" value="削除">
          </form>
        </td>
        <td class="cart_item_price">{{ $cart->item->price }}</td>
